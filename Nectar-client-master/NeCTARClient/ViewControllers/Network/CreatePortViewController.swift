//
//  CreatePortViewController.swift
//  NeCTARClient
//
...
// Create a new port for a network according to the parameters provided by the user
class CreatePortViewController: BaseViewController, UIPickerViewDelegate, UIPickerViewDataSource, UITextFieldDelegate{
    ...
    let adminStateP: [String] = ["UP","DOWN"]
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = "New Port"
        ...
        btn1.addTarget(self, action:#selector(CreatePortViewController.createClick),forControlEvents:.TouchUpInside)
        ...
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(returnBack), name: "PortCreated", object: nil)
    ...
    // Request Openstack to create a new port according to parameters
    func createClick() {
        ...
        if let user = UserService.sharedService.user{
            NeCTAREngine.sharedEngine.createPort(user.networkServiceURL, name: name.text!, networkId: networkId, adminState: singleAdminStateRef, token: user.tokenID).then{
                (json) -> Void in
                print(json)
                ...
                alert.addAction(UIAlertAction(title: "OK", style: UIAlertActionStyle.Default, handler: { Void in
                    ...
                    self.postNotification("PortCreated", obj: "created")
                }))
