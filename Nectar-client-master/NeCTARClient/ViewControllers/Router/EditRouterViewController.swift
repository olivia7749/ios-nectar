//
//  EditRouterViewController.swift
//  NeCTARClient
//
//  Created by Qi Qi on 18/9/18.
...
// Edit an existing router according to the parameters provided by the user
class EditRouterViewController: BaseViewController, UIPickerViewDelegate, UIPickerViewDataSource, UITextFieldDelegate{
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = "Edit Router"
        
        let btn1=UIButton(frame: CGRectMake(0, 0, 60, 30))
        btn1.setTitle("Edit", forState: UIControlState.Normal)
        btn1.setTitleColor(UIColor.blackColor(), forState: UIControlState.Normal)
        btn1.addTarget(self, action:#selector(createClick),forControlEvents:.TouchUpInside) //Click the button to start editing the router
        let item2=UIBarButtonItem(customView: btn1)
        self.navigationItem.rightBarButtonItem=item2
        
        
        name.placeholder = RouterService.sharedService.routers[routerIndex].name
        adminState.placeholder = RouterService.sharedService.routers[routerIndex].adminState
        externalNetwork.placeholder = RouterService.sharedService.routers[routerIndex].gatewayName
        ...
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(returnBack), name: "RouterEdited", object: nil)
    ...
    // Request Openstack to edit the router according to user settings
    func createClick() {
        
        var routertext = RouterService.sharedService.routers[routerIndex].name
        if (self.name.text?.isEmpty != true){
            routertext = self.name.text!
        }
        
        if (self.adminState.text?.isEmpty == true){
            singleAdminStateRef = RouterService.sharedService.routers[routerIndex].adminValue
        }
        
        if (self.externalNetwork.text?.isEmpty == true){
            singleExternalNetworkRef = RouterService.sharedService.routers[routerIndex].networkID
        }
        
        if (singleExternalNetworkRef == "-"){
            singleExternalNetworkRef = ""
        }
        ...
        if let user = UserService.sharedService.user{
            NeCTAREngine.sharedEngine.editRouter(user.networkServiceURL, name: routertext, external_gateway_info: singleExternalNetworkRef, admin_state_up: singleAdminStateRef, routerID: routerId, token: user.tokenID).then{
                (json) -> Void in
                ...
                alert.addAction(UIAlertAction(title: "OK", style: UIAlertActionStyle.Default, handler: { Void in
                    ...
                    self.postNotification("RouterEdited", obj: "edited")
                }))
